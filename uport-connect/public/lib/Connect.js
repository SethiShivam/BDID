...
import { isMobile, ipfsAdd } from './util1'
...
class Connect {
  ...
  constructor(appName, opts = {}) {
    ...
    this.network = config.network(opts.network)
    ...
    this.mobileTransport = opts.mobileTransport || url.send({
    ...
    this.registry = opts.registry || UportLite({ networks: config.networkToNetworkSet(this.network) })
  ...
  onResponse(id, cb) {
    ...
    const parseResponse = (res) => {
      ...
      if (util.isJWT(res.payload)) {
        console.log("Inside If JWT Condition")
        const jwt = res.payload 
        const decoded = decodeJWT(jwt)
        if (decoded.payload.claim) {
          console.log("Inside If Decoded condition")
   ...
   */
  async send(request, id, { redirectUrl, data, type, cancel } = {}) {
    debugger
    console.log({request, id ,redirectUrl, data, type, cancel})
    ...
    if (this.isOnMobile) {
      ...
      console.log("this.usePush && this.pushTransport------------->")
      this.pushTransport(request, { data }).then(res => this.PubSub.publish(id, res))
    } else {
      console.log("else------------->")
    var response =   new Promise((reject,resolve)=>{
console.log({data,cancel,})
         this.transport(request, { data, cancel }).then(result=>{
           console.log({result})
         resolve(result)
       }).catch(e=>{
         console.log({e})
   ...
   response = await response;
          console.log("Transport Response is ----->",response)
          debugger
          this.PubSub.publish(id, response)
       ...
       // .catch(err => this.PubSub.publish(id, err))
        ...
        // this.transport(request, { data, cancel })
        // .then(res => {
        //   console.log("Transport Response is ----->",res)
        //   debugger
        //   this.PubSub.publish(id, res)
        // })
        // .catch(err => this.PubSub.publish(id, err))
    }
  }
  ...
  async requestDisclosure(reqObj , id = 'disclosureReq', sendOpts) {  // 
    console.log("REQOBJECT IS ======>",reqObj)
    debugger;
    ...
    debugger
    console.log("createDisclosureRequest ====================>")
  

       this.credentials.createDisclosureRequest(reqObj, reqObj.expiresIn)
      .then(jwt => {
        console.log("JWT -------->",jwt,sendOpts)
        this.send(jwt, id, sendOpts)})
  ...
  async sendVerification(verification = {}, id = 'sendVerReq', sendOpts) {
    ...
    this.credentials.createVerification(verification).then(jwt => {
      const uri = util.paramsToQueryString(util.messageToURI(jwt), { 'callback_url': cb })
   ...
   */
  genCallback(reqId) {
    return this.isOnMobile ? windowCallback(reqId) : messageServer.genCallback()
  }
...
const windowCallback = (id) => {
  ...
  return util.paramsToUrlFragment(callback, { id })
}
