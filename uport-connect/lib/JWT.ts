...
export async function verifyJWT(
  ...
  const aud: string = options.audience
    ...
    : undefined
  var abc = decodeJWT(jwt)
  const payload = abc.payload
  const header = abc.header
  const signature = abc.signature
  const data = abc.data
  ...
  // var DIDAuthenticatorData = await resolveAuthenticator(
  //   header.alg,
  //   payload.iss,
  //   options.auth
  // )
  // const doc = DIDAuthenticatorData.doc
  // const authenticators = DIDAuthenticatorData.authenticators
  // const issuer = DIDAuthenticatorData.issuer
